...
if($_POST)
	{	
			...
			$query = "select * from edu where edu_id=:searchTerm or edu_name like :searchTerm order by edu_id";
    		...
    		$stmt->execute();
 			...
 			$query = "select* from edu order by edu_id";
			$stmt = $dbh->prepare($query);
			$stmt->execute();
...
<script>
function link(){
	window.location.href = "edu_insert.php"
	}
...
function resets(){
	...
	function deleteConfirm(id)
	{
		...
		if(result == true)
		{	
			this.document.deleteForm.action = "delete.php?id="+id+"&type=edu";
