...
prx.types.video = {
	...
	,onDisplay: function(item,containerid) {
		...
		case 'html5':
			cR += '<video width="100%" height="100%" '+ ((item.controls) ? 'controls' : '') + ((item.placeholder.url != '') ? ' poster='+getAssetUrl(item.placeholder) : '' )+' '+ ((!item.preload) ? 'preload="none"' : '') +' '+ ((item.autoplay) ? 'autoplay' : '') +' webkit-playsinline>';
